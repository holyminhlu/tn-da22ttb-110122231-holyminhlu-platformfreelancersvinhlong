    ...
    <section className="how-vlc-hero py-16 text-white md:py-20">
      <div className="how-vlc-container how-vlc-prose text-center">
        <p className="mb-3 text-sm font-semibold uppercase tracking-widest text-blue-300">
          {tUi("Hướng dẫn sử dụng")}
        </p>
        <h1 className="mb-5 text-3xl font-bold leading-tight md:text-4xl">{tUi(HOW_VLC_HERO.title)}</h1>
        <p className="text-lg leading-relaxed text-neutral-300">{tUi(HOW_VLC_HERO.subtitle)}</p>
      </div>
    </section>
  );
}
...
function RoleSwitch({
  ...
  return (
    <div
      ...
      role="tablist"
      aria-label={tUi("Chọn vai trò của bạn")}
    >
        ...
        return (
          ...
          >
            <span className="flex items-center gap-2">
              ...
              <span className="how-vlc-role-btn__label">{tUi(option.label)}</span>
            </span>
            <span className="how-vlc-role-btn__hint">{tUi(option.hint)}</span>
          </button>
        );
      })}
    </div>
    ...
    >
      {steps.map((item) => (
        <article key={item.step} className="how-vlc-step-card" data-step={item.step}>
          <h3 className="mb-2 text-lg font-bold text-[#1c2e4a]">{tUi(item.title)}</h3>
          <p className="mb-4 text-sm leading-relaxed text-gray-600 md:text-base">{tUi(item.description)}</p>
          <Link
            href={item.route}
        ...
        <div className="how-vlc-workflow-track">
          {WORKFLOW_STAGES.map((stage) => (
            <div key={stage.id} className="how-vlc-workflow-stage how-vlc-workflow-stage--active">
              ...
              <h3 className="mb-3 text-sm font-bold leading-snug text-[#1c2e4a]">{tUi(stage.title)}</h3>
  ...
  return (
    <section className="how-vlc-cta py-16 text-white md:py-20">
      <div className="how-vlc-container how-vlc-prose text-center">
        ...
        <h2 className="mb-4 text-2xl font-bold md:text-3xl">{tUi(cta.title)}</h2>
        <p className="mb-10 text-neutral-300">{tUi(cta.subtitle)}</p>
        <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link
            href={cta.primary.href}
            className="rounded-lg bg-[#0066cc] px-8 py-3.5 font-bold text-white transition hover:bg-blue-700"
          >
            {tUi(cta.primary.label)}
          </Link>
          <Link
